                    ...
                    while ((entry = jis.getNextJarEntry()) != null) {
                        if (!entry.isDirectory()) {
                            ...
                            if (fileName.endsWith(".class")) {
                                classes.add(
                                        Class.forName(fileName.substring(0, fileName.length() - 6).replace("/", "."))
//                                        defineClass(className, getBytes(entry, jis))
    ...
    protected void addURLToParentClassLoader(URL url) throws IntrospectionException {
...
//                ClassLoader.getSystemClassLoader();
    ...
    // This is a little ugly, but it works.  Perhaps a kinder, gentler method can be discovered. :)
